...
def sreach_guest(request):
    username = request.session.get('user', '')
    sreach_guest = request.GET.get("realname", "")
...
def sreach_phone(request):
    ...
    #print(search_name)
    data = Guest.objects.filter(phone__contains=search_phone)
    username = request.session.get('userinfo', '')
    return render(request,"guest_manage.html",{"username":username,"guests":data})
